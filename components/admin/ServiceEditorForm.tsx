...
export function ServiceEditorForm({ initialData, isEditing = false }: ServiceEditorFormProps) {
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      const { error } = await supabase.from('services').delete().eq('id', initialData.id);
      ...
      try {
        await fetch('/api/revalidate', {
          ...
          body: JSON.stringify({
            paths: ['/', '/services', `/services/${initialData.slug}`],
          }),
        });
      } catch (revErr) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <DeleteConfirmModal
        ...
        message={
          <>
            Are you sure you want to delete <span className="font-bold text-navy-900">&quot;{name || 'this service'}&quot;</span>?
            This will permanently remove the service from your website catalogue and navigation.
          </>
        }
